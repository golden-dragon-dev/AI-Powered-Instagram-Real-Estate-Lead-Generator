...
test("step 9b after budget asks area then type", async () => {
  ...
  assert.equal(areaTurn.stage, "qualifying");
  assert.ok(areaTurn.check.ok);
  assert.match(areaTurn.reply, /bedroom|type|studio/i);
  ...
  assert.equal(bedsTurn.matchCount, 1);
